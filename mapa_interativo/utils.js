...
// CPF/CNPJ FORMATTING
// ========================================
function formatDocument(doc, visible = false) {
    if (!doc) return '-';
...
window.toggleCpfVisibility = function (btn, docValue) {
    ...
    if (isHidden) {
        // Show
        valueSpan.textContent = formatDocument(docValue, true);
        btn.classList.remove('fa-eye');
        btn.classList.add('fa-eye-slash');
        btn.title = "Ocultar";
    } else {
        // Hide
        valueSpan.textContent = formatDocument(docValue, false);
